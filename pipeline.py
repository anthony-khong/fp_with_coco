...
def fit(*_coconut_match_to_args, **_coconut_match_to_kwargs):
    ...
    if not _coconut_match_check:
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def fit(Estimator(fit_fn), df) = fit_fn(df)'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))
        _coconut_match_err.pattern = 'def fit(Estimator(fit_fn), df) = fit_fn(df)'
        _coconut_match_err.value = _coconut_match_to_args
        raise _coconut_match_err

    return _coconut_tail_call(fit_fn, df)
...
def transform(*_coconut_match_to_args, **_coconut_match_to_kwargs):
    ...
    if (1 <= _coconut.len(_coconut_match_to_args) <= 2) and (_coconut.sum((_coconut.len(_coconut_match_to_args) > 1, "df" in _coconut_match_to_kwargs)) == 1) and (_coconut.isinstance(_coconut_match_to_args[0], Transformer)) and (_coconut.len(_coconut_match_to_args[0]) == 1):
        ...
        if not _coconut_match_to_kwargs:
            ...
            _coconut_match_check = True
    if not _coconut_match_check:
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " "'def transform(Transformer(transform_fn), df) = transform_fn(df)'" " in " + _coconut.repr(_coconut.repr(_coconut_match_to_args)))
        _coconut_match_err.pattern = 'def transform(Transformer(transform_fn), df) = transform_fn(df)'
        _coconut_match_err.value = _coconut_match_to_args
        raise _coconut_match_err

    return _coconut_tail_call(transform_fn, df)
