...
from pipeline import generic_estimator

@_coconut_tco
def one_hot_encoder_with_max_bins(column, weight_column, max_bins):
    return _coconut_tail_call(generic_estimator, fit_with_max_bins, transform, column, weight_column, max_bins)
...
def one_hot_encoder(column):
    return _coconut_tail_call(generic_estimator, fit, transform, column)
...
def transform(fit_dict, df):
    ...
    _coconut_match_to = fit_dict
    _coconut_sentinel = _coconut.object()
    if (_coconut.isinstance(_coconut_match_to, _coconut.abc.Mapping)) and (_coconut.len(_coconut_match_to) == 2):
        _coconut_match_temp_0 = _coconut_match_to.get('column', _coconut_sentinel)
        _coconut_match_temp_1 = _coconut_match_to.get('factors', _coconut_sentinel)
        if (_coconut_match_temp_0 is not _coconut_sentinel) and (_coconut_match_temp_1 is not _coconut_sentinel):
            column = _coconut_match_temp_0
            factors = _coconut_match_temp_1
            _coconut_match_check = True
    if not _coconut_match_check:
        _coconut_match_err = _coconut_MatchError("pattern-matching failed for " '"{\'column\': column, \'factors\': factors} = fit_dict"' " in " + _coconut.repr(_coconut.repr(_coconut_match_to)))
        _coconut_match_err.pattern = "{'column': column, 'factors': factors} = fit_dict"
